...
class SeedNode:
    ...
    def select_routers(
        ...
        logger.debug(
            f"Selected {len(selected)} routers from {len(candidates)} candidates (registry has {len(self.router_registry)} total)"  # noqa: E501
        )
    ...
    def _verify_pow(
        ...
        try:
            ...
            if leading_zeros >= required_difficulty:
                ...
                logger.warning(
                    f"PoW insufficient for {router_id[:20]}...: got {leading_zeros} bits, need {required_difficulty}"
                )
                return False
    ...
    async def handle_discover(self, request: web.Request) -> web.Response:
        ...
        logger.info(
            f"Discovery request: requested={requested_count}, returned={len(routers)}, preferences={preferences}"
        )
    ...
    async def handle_register(self, request: web.Request) -> web.Response:
        ...
        if not is_update:
            ...
            if not allowed:
                logger.warning(
                    f"Registration blocked by Sybil resistance: {router_id[:20]}... reason={reason}, source_ip={source_ip}"  # noqa: E501
                )
        ...
        logger.info(
            f"Router {action}: {router_id[:20]}... endpoints={endpoints}, regions={record.regions}{region_info}, source_ip={source_ip}"  # noqa: E501
        )
        ...
        if unique_reporters >= self.config.misbehavior_min_reports_to_flag:
            ...
            if avg_severity >= self.config.misbehavior_flag_severity_threshold:
                ...
                if router_id not in self._misbehavior_flagged_routers:
                    ...
                    router_flagged = True
                    logger.warning(
                        f"FLAGGED router {router_id[:20]}... for misbehavior: {unique_reporters} reporters, avg_severity={avg_severity:.2f}"  # noqa: E501
                    )
