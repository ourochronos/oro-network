...
class SeedRevocationManager:
    ...
    def get_revocation(self, seed_id: str) -> SeedRevocationRecord | None:
    ...
    def add_revocation(
        ...
        if existing and existing.timestamp > timestamp:
            ...
            logger.debug(
                f"Ignoring older revocation for {seed_id[:20]}... (existing={existing.timestamp}, new={timestamp})"
            )
            return True, None
        ...
        logger.info(
            f"Added seed revocation: {seed_id[:20]}... reason={record.reason}, issuer={issuer_id[:20]}..., source={source}"  # noqa: E501
        )
    ...
    def load_revocation_list_from_file(self, file_path: str) -> tuple[int, list[str]]:
        ...
        logger.info(
            f"Loaded {loaded_count} revocations from {file_path} (version={revocation_list.version}, errors={len(errors)})"  # noqa: E501
        )
