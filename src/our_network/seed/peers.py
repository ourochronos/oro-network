...
class SeedPeerManager:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            f"Seed peer manager started with {len(self.peer_seeds)} peers, gossip interval={self.gossip_interval}s"
        )
    ...
    async def _gossip_round(self) -> None:
        ...
        logger.info(
            f"Gossip round complete: {successful}/{len(self.peer_seeds)} peers exchanged, shared {len(routers_to_share)} routers"  # noqa: E501
        )
            ...
            try:
                ...
                if existing is None:
                    ...
                    if last_seen > existing_last_seen:
                        ...
                        merged += 1
                        logger.debug(
                            f"Updated router {router_id[:20]}... from gossip (peer={source_peer}, delta={last_seen - existing_last_seen:.0f}s)"  # noqa: E501
                        )
