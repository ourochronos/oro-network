...
class RateLimiter:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "unique_ips": len({e.source_ip for e in self._registration_events}),
            "unique_subnets": len({e.subnet for e in self._registration_events}),
...
class ReputationManager:
    ...
    def record_missed_heartbeat(self, router_id: str) -> ReputationRecord | None:
        ...
        logger.debug(
            f"Missed heartbeat: {router_id[:20]}... score={record.score:.3f}, missed={record.missed_heartbeats}"
        )
        return record
    ...
    def apply_penalty(self, router_id: str, penalty: float, reason: str) -> ReputationRecord | None:
        ...
        logger.warning(
            f"Reputation penalty: {router_id[:20]}... penalty={penalty}, reason={reason}, new_score={record.score:.3f}"
        )
        return record
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "below_threshold": sum(
                1 for r in self._reputation.values() if r.score < self.config.reputation_min_score_for_discovery
            ),
