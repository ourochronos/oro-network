...
class CorrelationDetector:
    ...
    def analyze_and_flag(self, router_id: str, timestamp: float, endpoints: list[str]) -> list[str]:
        ...
        if len(correlated_heartbeats) >= self.config.correlation_min_suspicious_events:
            ...
            logger.warning(
                f"Correlated heartbeats detected: {router_id[:20]}... correlates with {len(correlated_heartbeats)} other routers"  # noqa: E501
            )
        ...
        if similar_endpoints:
            ...
            for other_id, similarity in similar_endpoints:
                logger.warning(
                    f"Similar endpoints detected: {router_id[:20]}... and {other_id[:20]}... (similarity={similarity:.2f})"  # noqa: E501
                )
