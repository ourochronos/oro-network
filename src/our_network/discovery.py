...
class RouterInfo:
    ...
    asn: str | None = None  # Autonomous System Number for eclipse protection
...
class DiscoveryClient:
    ...
    last_successful_seed: str | None = None
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.post(url, json=report_data) as resp:
                        if resp.status == 200:
                            submitted = True
                            logger.info(
                                f"Submitted misbehavior report for router {report.router_id[:16]}... to {seed_url}"
                            )
                            # Track seed success
    ...
    def select_diverse_routers(
        ...
        count: int,
        exclude_subnets: set | None = None,
        exclude_asns: set | None = None,
    ) -> list[RouterInfo]:
        """
    ...
    def _get_router_subnet(self, router: RouterInfo) -> str | None:
    ...
    async def _query_seed(
        ...
        try:
            ...
            health.record_failure()
            raise DiscoveryError("Request timeout") from None
    ...
    def _get_continent(self, country_code: str | None) -> str | None:
    ...
    def reset_seed_health(self, seed_url: str | None = None) -> None:
        """
